...
def login_CSDN_one(request):
    s = requests.session()
    s.get(url=NVC_url, headers=headers)
    data = {"loginType": '1', "pwdOrVerifyCode": request.param['pwd'], "userIdentification": request.param['user_name'],
            "uaToken": uaToken,
...
def login_CSDN_two(user):
    ...
    response = seesion.get(login_url, headers=headers)
    # 输出响应码
    ...
    response = seesion.post(login_url, data=data, headers=headers)
    # 打印响应的状态码
    print(response.status_code)
    # 将响应的信息写入文件
    ...
    response = seesion.get('https://my.csdn.net/my/mycsdn', headers=headers)
    # 打印响应码
    print(response.status_code)
    # 将响应结果保存文件
